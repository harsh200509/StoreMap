      ...
      >
        ...
        >
              ...
              >
                ...
                {Array.from({ length: Math.max(1, aisle.divisions || 5) }).map((_, i) => {
                  if (aisle.orientation === 'vertical') {
                    ...
                    return i > 0 ? <line key={i} x1={aisle.x} y1={aisle.y + i*h} x2={aisle.x + aisle.width} y2={aisle.y + i*h} stroke={isHighlighted ? '#d8b4fe' : '#cbd5e1'} strokeWidth="1" className="pointer-events-none opacity-50" /> : null;
                  } else {
                    const w = aisle.width / (aisle.divisions || 5);
                    return i > 0 ? <line key={i} x1={aisle.x + i*w} y1={aisle.y} x2={aisle.x + i*w} y2={aisle.y + aisle.height} stroke={isHighlighted ? '#d8b4fe' : '#cbd5e1'} strokeWidth="1" className="pointer-events-none opacity-50" /> : null;
                  }
                })}
          ...
          {activeRoute && userLocation && (
            ...
            >
              {/* User Location Halo */}
              {useRealGPS && <circle cx="0" cy="0" r="60" fill="#3b82f6" opacity="0.1" className="animate-pulse" />}
